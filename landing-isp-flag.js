 ...
 * 落地 IP 检测（fork）
 *
 * 本 fork 由 enfplove 修改自折腾鲨 http_meta_geo.js：
 *   - 默认 api 改为 IPPure: https://my.ippure.com/v1/info（返回落地 IP 的 fraudScore 等）
 *   - 默认 format 改为: {{api.countryCode}} - {{proxy.name}} {{api.fraudScore}}
 *     结果示例: HK - 香港01 🇭🇰 85（评分为纯数字, 置于名称末尾）
 *   其余参数与用法与原脚本一致, 见下方原始说明。
 *
 * 适用于 Sub-Store Node.js 版 / Android root 模块（需 HTTP META 配合）。
 * App(Loon/Surge) 版请使用原 geo.js。
 *
 *
...
async function operator(proxies = [], targetPlatform, context) {
  ...
  let format = $arguments.format || '{{api.countryCode}} - {{proxy.name}} {{api.fraudScore}}'
  let url = $arguments.api || 'https://my.ippure.com/v1/info'
  let utils
  if (internal) {
